...
4. [Core Packages](#core-packages)
   ...
   - [toolkit/](#toolkit---additional-utilities)
...
3. Build final logger based on config
4. Register default Prometheus metrics
5. Connect to databases via `hooks.ConnectDB`
6. Ensure schema/indexes via `hooks.EnsureSchema` (if provided)
7. Wire shutdown signals (SIGINT/SIGTERM) to context
8. Build HTTP handler via `hooks.BuildHandler`
9. Start HTTP(S) server and block until shutdown
...
Groups HTTP/HTTPS port and protocol settings.
...
#### toolkit/cors/cors.go - Default CORS

**Location:** `/toolkit/cors/cors.go`
**Package:** `cors`

Pre-configured CORS middleware with sensible defaults.

#### Functions

##### `Default() func(http.Handler) http.Handler`

Returns CORS middleware with:
...
#### toolkit/windowsservice/programwindows.go - Windows Service

**Location:** `/toolkit/windowsservice/programwindows.go`
...
#### toolkit/windowsservice/programstub.go - Non-Windows Stub

**Location:** `/toolkit/windowsservice/programstub.go`
**Package:** `windowsservice`
**Build Tag:** `//go:build !windows`
...
*This documentation was generated for WAFFLE framework version based on commit 3815f34.*
