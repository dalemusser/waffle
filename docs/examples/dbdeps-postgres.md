...
WAFFLE already handles a controlled shutdown sequence. You just need to ensure `db.Close()` is called when the app is stopping. One possible pattern:

```go
// Somewhere appropriate in your app's shutdown handling:
...
Exactly where you call `CloseDB` depends on how you wire shutdown hooks in your app.  
The key idea is: **open the DB in ConnectDB, close it once on shutdown.**
...
[Postgres DBDeps with pgxpool](.examples/dbdeps-postgres-pgxpool.md).
