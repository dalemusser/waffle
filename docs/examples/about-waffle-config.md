...
> **Note on Let’s Encrypt http‑01 challenges**  
> When using Let’s Encrypt with the **http‑01** challenge, WAFFLE must listen on **HTTP port 80** so the ACME server can reach `http://<domain>/.well-known/acme-challenge/...`.  
