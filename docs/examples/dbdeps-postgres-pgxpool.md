...
**[Postgres in DBDeps (basic `*sql.DB`)](./dbdeps-postgres.md)**
...
As with `*sql.DB`, WAFFLE’s lifecycle provides a good place to do this.  
One pattern (you can adapt this to your shutdown wiring):
...
Call `CloseDB` once during shutdown, after WAFFLE has stopped accepting new requests.
