...
WAFFLE already provides a controlled shutdown sequence, so you simply add:
...
func (h *Hooks) OnShutdown(deps DBDeps) {
    if deps.MongoClient != nil {
        _ = deps.MongoClient.Disconnect(context.Background())
    }
}
```

(This assumes you add an `OnShutdown` hook — WAFFLE does not require one, but it is easy to wire in.)
