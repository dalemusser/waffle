...
This gives your feature full access to all DBDeps fields (`MongoDB`, `Postgres`, `MySQL`, `SQLite`, `Redis`, etc.).
...
func (h *Handler) ListUsers(w http.ResponseWriter, r *http.Request) {
    rows, err := h.deps.MySQL.QueryContext(r.Context(), "SELECT id, name FROM users")
    if err != nil {
        http.Error(w, "db error", http.StatusInternalServerError)
        return
    }
...
func (h *Handler) GetSetting(w http.ResponseWriter, r *http.Request) {
    var value string
    err := h.deps.SQLite.QueryRowContext(r.Context(),
...
Note: Use `router.New()` to get WAFFLE's standard middleware stack (request ID, logging, metrics, panic recovery, etc.).
